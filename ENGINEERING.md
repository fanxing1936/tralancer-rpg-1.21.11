...
签订新约时先解除已有七罪契约，把魔化归零，并令 `rpg_lt_auth` 固定为 100；此后
每刻清除失明与黑暗。因此“权柄完整度”不是另一根逐渐积累的污染条，而是新约生效
期间恒为完整的身份状态。普通长按与潜行长按在同一入口分流：
...
175 HP（700 的 25%）仪式保护线：伤害会真实扣除并显示，但不会越过保护线；旧约的
25% 斩杀在该阶段也被显式禁用。Boss 已绑定法阵时不再扣血，而是把十诫净界、
创世净光、伊甸敕界分别转化为 `+20 / +25 / +15` 稳定度，并给予施法者
`3 / 4 / 2` 点驱魔阅历。
...
技能反馈不直接写 Actionbar。`m59..m62` 只设置消息号和 40 刻寿命，统一 HUD 出口
再按优先级合成 `[十诫净界]`、`[创世净光]`、`[伊甸敕界]`、`[赦免]`。冷却使用
