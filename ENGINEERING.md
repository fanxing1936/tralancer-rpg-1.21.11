...
### 2026-08-27 · 圣所祷告、规范遗珍与领主层仪式补给

- `_prayer_pool.json` 为奖池唯一配置；10 枚规范货币一次，27 奖项以连续互斥区间覆盖 1–10000。五类总概率为 40% / 30% / 20% / 8% / 2%，详见 `PRAYER-POOL.md`。
- `_tools/rpg_catalogue.py` 从现有 give 目录提取完整组件，祷告、遗珍、领主宝库和仪式补给均引用规范物品，不重新拼装装备。新约/旧约等唯一剧情奖励不进入奖池。
- 祷告以个人 `rpg_pr_pending` 存储已付费的奖品索引，40 tick 金色光点/白色微光演出；不写 Actionbar，不建实体，不使用每 tick 大量粒子。动画中禁止重复扣费，空位不足保留待领记录；旁观/死亡暂停，断线与 reload 不重抽。
- 修复旧货币掉落漏 `currency_tag`：六张表追加 `set_custom_data`，菜单按完整旧名与 lore 精确迁移。`prayer/currency` 的旧文本是匹配条件，`polish_demon_names.py` 不得改写这些条件。`extract_loot.py` 同时兼容 SNBT 字符串和对象格式的 custom_data。
- `_endless_supplies.json` 定义七档遗珍、四档领主宝库及七柱仪式补给。进入领主战先补足图腾1、圣水2、天启星1、银钉4、守御粉笔3、告解铃1、封魔灯1、净罪香2及本柱媒介4。以实例 ID + 层数记配额；无空位时不发放、不记已领，可手动重试。不会赋予调查/见证进度。
- `add_drop_policy.py` 在战斗 tick 前处理首次见到的生物：98 张原版实体掉落表与整组充能苦力怕头颅表为空，保留七张本包实体表和35名罪仆表。普通装备掉率归零；佣兵主手改用专门的 `minecraft:slots` 表原样归还。禁止使用全局清物品或关闭 doMobLoot 的方案。凋灵之星、坐骑内部库存等硬编码例外详见 `DROP-POLICY.md`。
- 构建顺序：`add_endless_exorcism` → `add_endless_supplies` → `add_prayer` → `add_drop_policy` → `make_boxes` → `polish_demon_names`；补齐源码构建、图鉴生成与调试手册入口，重建不丢补丁。
- 静态回归：`check_prayer_supplies.py` 穷举10000抽取点、核对27奖品原始组件、七柱补给保护条件及文档；`check_drop_policy.py` 验证白名单与安全边界；现有章节、神约、罪仆、仪式、退役生物与UI检查均通过。
- 真实1.21.11隔离服使用协议测试玩家验证：普通粗金拒收、旧币迁移、恰好扣10、重复点击不重扣、40次动画推进结算、满包不收费/待领恢复、旁观暂停、reload保留；实际第5层自动补给、其余六柱媒介、满包补领和退出再加入不重复发放均通过。协议测试不等同于人工游戏画面审美验收。
- 独立复查补上死亡界面的补给拦截，并以生存玩家 Health=0 实测不会消耗层配额；旧副本的柱位由当前层数恢复，更新中途无需重开副本。部署前备份位于 `backups/2026-08-27_prayer_and_ritual_supply/`，含三个存档旧包与实例/工程资源包；资源包内容未变。
...
`rpg:command/give/*` 一件件发会把背包塞爆好几轮。`make_boxes.py` 会在所有玩法
生成器结束后重新扫描最终发放目录，把当前构建打包成 `rpg:command/give/box`。
现在一条命令给出 **14 只潜影盒**，每盒最多 27 个槽位：
...
前面几部分一直用 `hotspots.py` 的静态计数当性能指标。这一次改成**在服务器上
量真实的每刻耗时**（`/tick query`），结论和静态模型讲的不是同一个故事。

## 30.1 先说三次量错

这一轮前后量了十几次，其中三次的数字是**我自己制造的假象**，值得记下来：

* **JIT 冷启动。** 会话里第一次 `tick sprint` 得到 2.50 ms/刻，后面稳定在
  0.13–0.36。第一次跑的是没编译过的字节码，不能当数
* **`/debug start` 自己很贵。** 开着 profiler 量到 4.42 TPS，我差点当成
  包的开销。它测的是 profiler 自己
* **`datapack disable` 会写进 level.dat。** 为了取基线禁用过一次之后，
  **后面每一次运行都在没有数据包的情况下跑** —— 中间一整批数字全是废的。
  最后改成每次量之前删掉整个存档重建

教训很简单：**基准测试要么每次从干净状态开始，要么就别信它。**
...
1. **召出来的变种没有"已处理"标记**，于是它自己也算"新出生的"，
   下一刻被批处理捞起来**再掷一次点** —— 一代接一代
2. **原本那只没被杀掉**，所谓"替换"实际上是"追加"

僵尸那一支每处理一只，期望再产出 0.75 只未标记的僵尸（掷点 8 出 1 只、
9 出 2 只、10 出 2 只、14–15 出 5 只，除以 20）。0.75 < 1 所以不会爆炸，
但收敛值是 1/(1-0.75) = **4 倍**。

无头实测（每次从干净存档开始，投放 40 只僵尸）：

| 版本 | 稳定后的实体数 | ms/刻 |
|---|---|---|
| 原始 | 204 | **5.4** |
| 打上已处理标记 + 杀掉原本那只 | 61 | 2.3 |
| 再加 30.3 的外提 | 56 | **1.9** |

无数据包的基线是 0.2 ms/刻。也就是说：**这个包大部分的开销，不是它的命令
本身贵，而是它把世界里的生物变多了三到四倍。**
...
`as @e` 是世界上**每一个**实体；对其中每一个，`if entity @e[tag=devil]`
**又要走一遍全表**。83 个实体就是约 6900 次选择器求值，两行接近一万四。

而那个判定问的是"世界上有没有恶魔"，**与当前是哪个实体毫无关系** ——
每一轮的答案都一样。提到循环外面即可：没有 Boss 的常态下（也就是绝大多数
时间）它从 O(n²) 直接变成 O(1)。

`opt_guard` 没抓到它，是因为那个 pass 找的是**行首**就能守卫的形状，
而这里的判定夹在 `as @e` 和 `at @s` 中间。新加的 `opt_hoist.py` 专门找这种
"夹在中间、且与 @s 无关"的存在性判定，安全条件是内层选择器不带任何
位置或排序过滤（`distance` / `sort` / `limit` / 坐标）—— 带了就说明它的答案
依赖上下文，一律不动。

全包扫下来只有这两行，但它们正好在最热的位置上。

## 30.4 静态模型为什么没看出来

`hotspots.py` 数的是**命令条数与遍历次数**，而这两个指标对上面两个问题都
几乎没有反应：

* 滚雪球根本不改变任何一行代码，它改变的是**世界里的实体数** ——
  静态分析看不见运行时的实体
* O(n²) 那两行在静态计数里就是"两条命令、两次遍历"，
  代价的平方项藏在运行时

所以这一轮之后，性能结论以 `/tick query` 的实测为准，`hotspots.py`
退回它擅长的位置：找出**结构上**可疑的写法（不限距的选择器、按人数放大的
入口、没走索引的遍历），而不是当作耗时的代理指标。

## 30.5 开销与验证

* **5.4 → 1.9 ms/刻（降 65%）**，实体数 204 → 56
* 静态指标：空闲遍历 283 → **280**，最坏情形 533 → 531 —— 几乎没动，
  这正是 30.4 那一节的意思
* 每多一个玩家的固定开销仍是 **1 次全表走查/刻**
* 功能未受影响，无头实测：`damage_scan` 仍能标记命中 ✓、
  佣兵仍能召出 ✓、变种系统仍在出货（30 只僵尸滚出 2 只铁剑僵尸、
  4 只僵尸村民）✓、服务器零抱怨
...
这一轮全部来自作者的实机反馈。共同点是：`validate.py` 不报、服务器不报、
无头测试也测不到 —— 因为它们要么需要**玩家**，要么根本不是"错误"，
而是**条件恒真**或**条件恒假**。

## 31.1 放任何方块都会立起一支图腾

驱魔图腾走 `minecraft:item_used_on_block`，我照着 `using_item` 的样子写了：

```json
"conditions": {"item": {"predicates": {"minecraft:custom_data": "{totem_tag:1b}"}}}
```

**这个触发器没有 `item` 字段。** 原版把物品判定塞在 `location` 里的
`minecraft:match_tool`（见 vanilla 的 `revaulting.json`）。我写的那个键被整个
忽略，条件于是变成**空的** —— 而"对方块使用物品"把**放方块**也算在内。
于是每放一块石头就立一支图腾。

## 31.2 而拿着图腾右击却毫无反应

同一个触发器只在交互**成功**时才响。不死图腾没有任何 `useOn` 行为，
右击方块直接 PASS —— 拿着正主反而不响。

所以两个症状是**同一个根因的两面**：只把条件写对，图腾会从"到处乱立"
变成"永远不响"。改用 `using_item`（长按右键）才两个都治，
顺带和包里其余所有主动物品统一了手感。

再加一道保险：`rite/trigger` 自己验一遍手里拿的是什么。
这类天生promiscuous的触发器，正确性不该全押在进度的断言上。

**新增 `check_adv.py`：拿原版当字典。** 扫描客户端 jar 里 1584 个原版进度，
统计每个触发器实际用过哪些条件字段，再拿本包的去比。原版没用过的键就报出来。
这个错本来就该在构建期被抓住，而不是靠在游戏里放一块石头。

## 31.3 不同的 actionbar 互相盖

驱魔那一版立过规矩：**屏幕下方那条 actionbar 全局只有一个出口**。
之后加的契约、佣兵、人偶、天启星、仪式、空缺者……一共 19 处又开始各写各的，
于是每刻都在渲染的进度条和一次性提示互相冲。

`opt_actionbar.py` 不是手工改那 19 处，而是**扫描 + 改写**：找出所有
`title <选择器> actionbar <文本>`，去重编号，调用点换成"往消息槽里写一个号"，
再由 HUD 按优先级渲染（提示 2 秒 > 蓄力条 > 状态条）。
以后新加的提示只要还是这个写法，下一次构建就会被一起收编。

一个自己踩的小坑：那条把"没有提示占着"加进渲染分支的正则顺手也改了
**末尾的寿命递减** —— 那会让提示占着的两秒里蓄力条不老化，提示一散就残留
一条过期的条。正则收窄到只改跳去渲染函数的行。

## 31.4 空缺者整套体系是隐形的

作者说"空缺者的内容没有生效"。三条机制（蔓延／撕壳／附身转移）代码都对，
无头测试也逐条验过。问题在**前提**：

空缺者的显形挂在 `rpg.h.holy_weapon_tag1`（手里握着圣器）上，而
`holy_weapon_tag` 全包**只有一个来源** —— `item_modifier/command/holy.json`，
也就是武器分支系统（丢一张神圣唱片给武器加分支）。

也就是说：要看见一个空缺者，你得先拿到唱片、给武器加过神圣分支、并且正握着它。
在那之前空缺者**全程隐形** —— 蔓延、撕壳、附身转移全都在照常运转，
只是一个也看不见，于是整套体系看起来像没生效。

**这不是判定写错了，是这个前提没有可获得的满足方式。**
修法是让本来就该算圣器的东西真的算：驱魔图腾、驱魔圣水、圣水、天启星、
替死人偶，五件都补上 `holy_weapon_tag`。顺带接对了另外两件事，而且都合乎设定 ——
握着圣物时魔化**自然消退**；魔化到 91 以上时圣物**会灼手**。

## 31.5 三件老驱魔道具本来一行逻辑都没有

它们早写着驱魔的文案，却谁也没接上：

* **替死人偶**　只是召出一只发光不动的悦灵，Lore 承诺的「使祂显形」零实现。
  现在：12 格内的空壳不必持圣器也显形；而 16 格内的你每一轮沾上的魔化
  **改由人偶承受**（按实际净增量精确转嫁），吃满 10 点就碎 ——
  全包唯一能挡住魔化的东西
* **圣水**　同样零逻辑，而且仪式那边只判"附近有没有 `area_effect_cloud`"，
  于是**任何**滞留药水都能点燃图腾。现在按 `custom_color` 给药水云验明正身
  （验一次就挂标记），仪式改判这个标记；云本身每秒洗 1 点魔化并灼烧空壳
* **天启星**　原本只照 `devil` 标签、半径 7 格。现在半径 32，
  照得出空壳与魔化 31 以上的人，对空壳直接审判

## 31.6 顺带：和魔神解约

契约原本只有逆圣化一条出路，而逆圣化要求魔化先推到 100 ——
"想退出契约，得先把自己弄得更脏"。对早期签错柱位的人那不是出路，是死胡同。

第二条路复用已有的器械：**立一支驱魔图腾、浇上圣水，在它燃着时长按你那本
已立约的书**。仪式本来就是烧污染的，用它烧断契约顺理成章，成本也实打实。
代价：当场 +20 魔化、凋零 10 秒，图腾一并烧尽；书退回未立约，可以改投别的柱位。

## 31.7 开销与验证

* 空闲遍历 **283**，与上一部分持平；最坏情形 531
* 每多一个玩家的固定开销仍是 **1 次全表走查/刻**
* 函数 357 → 399，进度 39
* 无头 1.21.11 零抱怨，逐条验过：空手不再立起图腾 ✓、
  毁约后属性回退（26 → 20）且柱位清零、图腾一并烧尽 ✓、
  消息槽正确武装 ✓、圣水云被认出而普通滞留药水被排除 ✓、
  普通药水点不燃图腾 / 圣水点得燃 ✓、
  **人偶 + 圣水让空壳曝光度冲到 97 并成功裂壳放出碎片（全程没有神圣分支武器）** ✓
* 四个给予函数全部合法

这一部分的教训写在标题里：**"代码是对的"和"这件事在游戏里会发生"是两回事。**
31.1 是条件恒真，31.4 是条件恒假 —— 两者都不会让任何检查器出声。

---

# 32. 继承收尾：契约 HUD、多人隔离与旧武器现代化

本轮工作从 Claude 留下的生成器体系继续，仍坚持一条原则：**改生成器，不手改最终包**。
因此下面各项都接入 `_tools/build.sh`，从 `_orig/` 重建时会完整复现。

## 32.1 契约对象常驻 HUD

actionbar 现在分成三个互不覆盖的 storage 片段：状态、契约对象、契约冷却。
只要玩家已经立约，中段就常驻显示对应柱位；发动契约能力时，冷却条只占用右侧片段，
不会再把“我和谁签了契约”一起抹掉。柱位名称直接由 `add_pact.py` 的七柱定义生成，
避免玩法逻辑与 HUD 文案各维护一份。

## 32.2 真正按玩家隔离的多人逻辑

这次不只修选择器表面上的 `@p`：玛门箭矢和物品回收绑定真实射手，圣化仪式使用
唯一仪式编号隔开同地并发玩家，小队移交与近距点火先按小队号圈定候选人。
旧武器命中统一从受击实体用 `on attacker` 追溯攻击者，冷却、连段、毒层与标记均归
玩家自己，不再用 `reset *` 清掉全服状态。

原版恶魔 Boss 栏改成四个固定槽位。Boss 获得稳定槽号后，只写自己的血量、名称与
附近观众；死亡或失效时释放槽位。这样远处同时开战不会再由最后遍历到的实体覆盖
同一条栏。第五个并发 Boss 采用确定的溢出策略，等待空槽而不污染已有战斗。

## 32.3 十七张旧武器卡全部现代化

升级前的十七件武器按复杂度拆成两个生成器：
`modernize_legacy_weapons.py` 负责统一命中入口、蓄力、投射物归属与玩家状态；
`modernize_legacy_advanced.py` 负责随机连段、四景连击、筋斗云、教条战斧纹饰和
flame / sweep / wind 老镶嵌。新的实现沿用后来武器的“小入口 + 独立 cast/hit 函数
+ HUD 消息槽”结构，攻击伤害也明确写出 `by @s` 或真实施法者。

十七张原卡的名称、叙事句、颜色和版式保留，只重写已经不符合实际机制的效果句。
新文本会说明蓄力时长、范围、连段、标记和代价；生成器同时断言 17/17 均命中且
新句唯一，避免以后重建时静默漏掉某一张卡。

## 32.4 圣器保护与空缺者随机柱位

主手、副手或四个护甲槽任一携带圣器，玩家都会进入 `rpg.holy` 状态；tick 尾统一
清除失明与黑暗，因此无论效果来自原版、生物还是本包能力，都不能在圣器持有者身上
残留。检测仍复用三代 `holy_weapon_tag`，兼容旧存档里的圣器物品。

无宿主恶魔的诞生柱位则改为独立掷 `1..7`。这个随机数不再读取附近玩家的契约，
所以玩家与某柱立约不会让野外空缺者偏向同一位魔神。

## 32.5 无地形破坏的伪爆炸

旧包用 `Fuse:0` 苦力怕表达瞬爆；早期迁移曾把它换成 `fuse:0` TNT，虽然解决了
延迟，却会真的炸掉地图。现在 `instant_boom.py` 只识别这批明确的攻击入口，改为
爆炸粒子、声音和内外两段距离伤害，整个过程不生成苦力怕或 TNT，也不执行任何
方块命令。普通非零引信 TNT、TNT 矿车和玩家放置的 TNT 不在改写边界内。

有可靠来源的入口会保留归属：疾风弓从箭矢 `on origin` 找射手（射手已不可用时退回
无主冲击），胸甲殉爆绑定穿戴者，Boss 技能在切换到玩家或侍从前先锁定当前 Boss。
伤害源使用同步生成、同刻清除的爆心 marker，`by` 保留从爆心向外的击退方向，`from`
保留真实施法者。二阶段侍从继承独立于血条槽位的 Boss 唯一编号；即使两场战斗贴近，
或侍从追出十二格，也只会由自己的 Boss 引爆并回收。降临者死亡探针本身不是合法
施法者，因此只使用位置型伤害，不伪造击杀者。

command damage 没有原版爆炸的方块暴露度采样，因此这里采用确定的内外两环衰减；
这是“绝不改方块”换来的明确规则，而不是暗中继续召一枚不可见 TNT。
`check_creeper.py` 在构建末尾阻止攻击苦力怕或零引信 TNT 重新混进最终包。

## 32.6 Boss 热路径与全域运行时优化

`opt_runtime_hotpaths.py` 在所有内容生成器之后执行，以 Claude 已采用的“空标签守卫、
类型索引、循环反转”为基础，再把高频逻辑改成一次绑定实体、局部函数处理。Boss 一、
二阶段不再让每条技能命令各扫一遍全世界；血条状态以 10Hz 刷新，死亡探针仍逐刻释放
槽位。斩击实体只存活 20 刻，侍从最多六只并在 1200 刻后回收，避免持久实体堆积。

玩家等级生命修饰只在等级变化时重建，手持属性读取改为每 5 刻一次；旧武器命中反馈
空闲时只有一个受伤标签守卫。普通箭不会进入技能箭热路径，三种旧技能箭带 200 刻
硬回收；深海追猎、恶作剧和裂隙箭均按箭实体一次绑定。高密度罪约/Boss 粒子保留主色
与轮廓层，但限制单次粒子量并从强制全距离发送改为普通可见距离。

双层 Actionbar 使用独立的 `rpg:combat_prompt` 私用区字体：上层恶魔出招提示烘焙
为 28 像素单元、27 像素 ascent，实际内容置于行内 y=4，使其相对下层状态真正上移
四像素；负 advance 将光标送回起点后再绘制原有动态状态，二者保持在同一 Actionbar
消息中，因此不会互相覆盖。单个提示按两个 192 像素安全字形拼合；正文采用 11 像素
统一粗体并将 alpha 边缘量化为 0/255，避免 GUI 整数缩放把半透明边缘放大成灰色毛边。

七柱普通招式池由每位三招扩为五招，总计 35 个普通技能；新增招式不生成持久场地实体，
只在施放刻执行局部玩家选择器、位移、状态与粒子，因此不增加常驻 tick 路径。实体自身的
`rpg_dm_last` 记录上一招，随机撞号时顺延并回绕，保证同一领主不会连续重复相同技能。
双层提示图集同步扩为 44 行（35 个普通技能、无名者 1 招、8 个罪约）。

## 32.7 真名调查与四阶段驱魔

七柱降临者统一为 700 最大生命，并把存续窗口扩到十分钟。玩家只有携带圣器亲历
某柱五种招式中的任意三种，才会分别取得三份不可重复罪证；三证齐全后获得持久的
`rpg.name.N` 真名确证与该柱弱点媒介。招式钩子写在最终 `skN_1..5` 入口，重复招式
由玩家标签拦住，因此多人各自记录，不会共享或抢走调查进度。
`rpg_caseN` 只作为 HUD 显示缓存：每次取得新罪证后都由五个实际 `rpg.clue.N.I`
标签重新计算，旧存档残留的计分板数值无法再让第一份新罪证直接快进到真名确证。
真名本身仍按设计跨战斗保留；需要从头验证时，玩家可主动执行
`rpg:inquest/reset_self`，该函数不会被加载或战斗逻辑自动调用。

领主在 420 生命进入镇压锁血；真名持有者靠近燃烧图腾后进入显形，燃烧图腾本身
提供仪式圣性，不再要求玩家额外持有另一件圣器。图腾和
领主共享单调递增的 `rpg_rite_id`，所以相邻法阵、多只领主也不会串台。四格内投入
该柱媒介时用 1.21.11 的 `if items entity @s contents` 检测，并且只消耗一件。进入宣判
后，持名者在六格内连续守阵五秒转入逐离；成功先回收死亡探针再杀死领主，
不会误触原有死亡伪爆炸。法阵熄灭或失联则把领主恢复到 560 生命并要求重新镇压。
`add_true_name_rite.py` 在热路径优化之后运行，专门接入最终版 `advent_tick` 与
`rite/beat`，并自检 7 个 700 生命召唤、35 个罪证钩子及唯一入口。

## 32.8 反仪式、裁决与驱魔师成长

活动法阵以 `rpg_ex_stab=100` 为稳定度，七柱各自从同一个受限冷却入口进入反仪式；
伪名与妒影只在对应事件期间生成，并带类型、标签、超时与显式清理。其余反仪式直接
在当前法阵上下文结算，不增加无条件全世界遍历。掉落工具先按物品组件分类成短命候选
标签，再选择最近匹配项，普通掉落物不会挡住媒介或粉笔检测。

第四阶段改为 15 秒裁决窗口：消灭恢复正常 Boss AI 并由原死亡乘客探针结算武器核心；
放逐复用安全的仪式收尾；封印消耗封魔灯并生成带柱位的遗物；契约调用现有七柱书籍
生成函数。封印遗物的背包检查合并进现有玩家索引，只有实际携带者进入十分钟计时，
不会让所有玩家每刻进行七次物品扫描。

驱魔师成长使用独立 `rpg_ex_xp / rpg_ex_lvl / rpg_ex_path`，不改普通经验。玩家每刻
入口复用已有 `taint/taint` 的一次玩家遍历；只有阅历改变时才重算等级。三路线分别
影响打断与消灭、稳定度减损、宣判速度；五级法阵从一个粉笔槽位提升为两个。
`add_exorcism_expansion.py` 必须紧随 `add_true_name_rite.py`，负责生成全部状态机、工具、
职业菜单以及反仪式实体的清理路径。

---

# 33. 卡巴拉生命之树、上下两约与统一文本

生命之树原本在十源质归位时直接发放一份笼统的“新约”，物品、聊天、HUD 与玩家
面板又各自维护名称和颜色。这次把它改成一条可追踪的两阶段进程：**十源质完成 →
旧约 → Daath 汇聚 → 新约**。实现仍由生成器落地，最终 `rpg/` 只作为构建产物。

## 33.1 获得与转化状态机

`add_life_tree_particles.py` 负责贴地法阵，`add_kabbalah_covenant.py` 负责卡巴拉血契、
十源质及其归位输入；`add_divine_covenants.py` 最后接管完成奖励和 Daath 转化。

1. 玩家长按 `[秘仪]卡巴拉血契`，以自身朝向展开生命之树；十枚 `[源质]` 只有在
   名称对应的圆心内长按才会嵌入，错误位置与重复输入都不消耗物品。
2. 第十枚源质归位后，树锚获得 `rpg.lt.complete`。最近的仪式主持者若尚无
   `rpg_lt_claim`，便得到唯一一次 `[契约]旧约`；领奖标记随后永久置为 1，重复展开
   或重复完成不会复制奖励。
3. 完成状态下，玩家站入隐藏的 **Daath 节点**圆心并手持 `[秘仪]真·十字架`
   长按右键。输入会立即消耗十字架并绑定主持者；随后 24 刻内，十件源质展示物逐刻
   向 Daath 汇聚。
4. 汇聚完成时清除主持者背包中的旧约及旧约签订状态，发放未签订的 `[契约]新约`，
   并回收十源质、真·十字架展示物与生命之树锚。树的粒子完全依附锚点，因此锚点
   消失即代表整棵树收束，不留下常驻粒子任务。

开发与实机探针统一用 `/function rpg:ritual/life_tree/give/all` 取得卡巴拉血契、十源质
和真·十字架；普通领取箱则由最后运行的 `make_boxes.py` 收录这些生成物。旧版存档中
已经领取的单阶段新约会由迁移入口清掉并重铸为旧约，避免绕过 Daath 阶段。

## 33.2 旧约：上半部律法

未签订与已签订状态分别使用 `[契约]旧约`、`[已立约]旧约`。签订后每刻推进
`rpg_lt_regen`；达到 400 刻时给予 1 秒生命恢复 I 并归零，所以真实周期为 **20 秒**。
魔化仍照常存在，但每次两秒批量结算结束后固定移除 2 点，且最终值不会低于 0。

`[十诫净界]` 把 `rpg_lt_div_cd / rpg_lt_div_max` 同时置为 600 刻，即真实冷却
**30 秒**。技能对 10 格内所有 `rpg.demon`、`rpg.demon.minion` 与独立标记的
`rpg.demon.fly` 结算：当前生命不高于最大生命的 25% 时直接斩杀，否则造成最大生命
20% 的神圣伤害。伤害通过短命 `rpg.divine.cast` 施法者标签保留归属，不会把多人
同时施法的伤害记给邻近玩家。

## 33.3 新约双式与赦免借力

签订新约时先解除已有七罪契约，把魔化归零，并令 `rpg_lt_auth` 从 100 起始；此后
每刻清除失明与黑暗。权柄现在是可消耗资源：不足 100 时由 `rpg_lt_auth_t` 每 40 刻
恢复 1 点，消费后计时重新开始。普通长按与潜行长按在同一入口分流；潜行判断统一
使用项目已有的 `rpg:sneaking` 实体谓词，不再读取不可靠的玩家 `Pose` NBT：

| 入口 | 函数与刻数 | 实际结算 |
| --- | --- | --- |
| 普通长按新约 | `invoke_new_beam`，400 刻 / 20 秒 | `[创世净光]` 沿视线前进 20 格，每段搜索 2.5 格内恶魔；每个目标只命中一次，受到 25% 最大生命值伤害与 15 点基础伤害，并获得 8 秒发光、6 秒虚弱 I |
| 潜行长按新约 | `invoke_new_field`，600 刻 / 30 秒 | `[伊甸敕界]` 作用于 8 格内恶魔，造成 15% 最大生命值伤害与 10 点基础伤害，并施加 8 秒缓慢 III、虚弱 II、发光；同时清除范围内非旁观玩家的失明、黑暗、凋零、中毒、缓慢和虚弱，给予 6 秒生命恢复 II、抗性 I、伤害吸收 II |
| 手持七罪契约书长按 | `borrow`，300 刻 / 15 秒 | `[赦免]` 按书中 `pact:1..7` 直接调用同一柱的 `p1..p7`，不进入七罪签约／发动入口，不增加魔化，也不承受契约枷锁 |

三种新约用法以及旧约净界共用 `rpg_lt_div_cd`，不是四条可并行恢复的冷却。
`rpg_lt_div_max` 记录本次起始刻数，让 HUD 能区分 300 / 400 / 600 刻技能并按真实比例
回填。这个共享锁也阻止玩家先放敕界、再立刻换七罪契约书借力。

创世净光使用一次性 `rpg.divine.hit` 去重：二十个前向采样点重叠时，同一目标仍只
承受一份百分比伤害。伊甸敕界则是瞬时八格领域，不生成持续场地实体；六秒增益与
八秒压制由原版效果计时，所以没有新增常驻实体或逐刻范围扫描。

玩家面板“契约与权柄”页还提供两项不占用共享冷却的权柄分授：

| 面板入口 | 消耗与时限 | 实际结算 |
| --- | --- | --- |
| `[终末圣裁]` | 25 权柄；`rpg_lt_judge=600`，30 秒内等待下一次恶魔命中 | 复用既有 `rpg.hurt` 与 `on attacker`，不新增世界伤害扫描；命中时剥离目标的恢复、抗性、吸收、力量、速度、隐身与防火，附加 20% 最大生命值 + 10 点神圣伤害，生命不高于 20% 则直接斩杀 |
| `[圣子恩赐]` | 35 权柄；12 格内最近一名其他非旁观玩家 | 清除失明、黑暗、凋零、中毒、缓慢与虚弱，立即治疗并给予生命恢复 II 10 秒、伤害吸收 II 20 秒、抗性 I 8 秒；无合法目标时提前返回且不扣权柄 |

圣裁只在命中 `rpg.demon`、`rpg.demon.minion` 或独立 `rpg.demon.fly` 时消耗覆印，
普通生物不会浪费准备状态。多人归属由受击实体短命标签、`on attacker` 与
`rpg.divine.cast` 串联，不用“附近最近玩家”冒认施法者。

三种净化攻击统一使用 `rpg:divine_light`，并将其并入护甲、抗性、无敌帧、附魔与
盾牌穿透标签，避免“百分比伤害”被不同 Boss 防御配置改写。真名调查第一阶段仍保留
175 HP（700 的 25%）仪式保护线：伤害会真实扣除并显示，但不会越过保护线；旧约与
圣裁的斩杀在该阶段也被显式禁用。Boss 已绑定法阵时不再扣血，而是把十诫净界、
创世净光、伊甸敕界、终末圣裁分别转化为 `+20 / +25 / +15 / +20` 稳定度，并给予施法者
`3 / 4 / 2 / 3` 点驱魔阅历。

## 33.4 签约、解约与物品状态

契约书的交互有三层明确顺序：未立约时签订；已立约但手持旧副本时先补发标准
`[已立约]` 版本；最后才检查解约或施法。这样旧存档、复制出的未盖印书和正常获得的
书都会收敛到同一组件集合。

旧约与新约共用一种无惩罚解约方式：玩家手持对应的 `[已立约]` 契约书，在 6 格内
燃着的驱魔图腾旁长按右键。入口把书恢复为 `[契约]`，清空上位契约状态、权柄与冷却，
并消耗最近图腾的燃烧展示。它不复用七罪契约的 `+20` 魔化与凋零反噬；两套毁约
规则必须在文档和 Lore 中保持区分。

## 33.5 与全包一致的 UI 语法

卡巴拉与神约不再另造一套 `[神约]`、`[圣遗物]` 文本。最终语义只有以下四类：

| 对象 | 标准前缀与名称 | 主要色 |
| --- | --- | --- |
| 卡巴拉血契、真·十字架 | `[秘仪]卡巴拉血契`、`[秘仪]真·十字架` | 秘仪紫 `#D596F2` |
| 十源质 | `[源质]01 · 王冠` 至 `[源质]10 · 王国` | 秘仪紫为类目，源质本身保留辨识色 |
| 未签订上位契约 | `[契约]旧约`、`[契约]新约` | 旧约圣金 `#D4AF37`；新约权柄青 `#62D9E8` |
| 已签订上位契约 | `[已立约]旧约`、`[已立约]新约` | 同上；本名统一白色且不加粗 |

方括号前缀、Lore 标签和技能名加粗，物品本名与正文不加粗；全部显式
`italic:false`。Lore 复用七罪契约的“来源与操作 → 分隔线 → `🜏` 能力层级”结构，
说明文字用灰色，只让系统名词进入圣金、秘仪紫与权柄青色板。聊天只保留
`[秘仪]`、`[旧约]`、`[新约]` 三个流程前缀，Daath 是正文中的节点名，不再成为
一套独立系统前缀。

技能反馈不直接写 Actionbar。`m59..m65` 只设置消息号和 40 刻寿命，统一 HUD 出口
再按优先级合成四项持物权能与圣裁覆印、圣子恩赐、圣裁命中。冷却使用
与七罪契约相同的五格 `▰ / ▱` 条，放进右侧 storage 片段；冷却未结束时只播放拒绝
音，不刷聊天。新约状态用十格权柄条取代魔化 / 圣痕段，玩家面板标题也从
“契约与侵蚀”切换为“契约与权柄”，并先显示“上位契约：新约 · 权柄”。

所有物品来源——初次 Loot、签约替换、补发、解约回退——必须调用同一份契约组件
生成器。否则初次获得与解约后的同名书会再次出现颜色、Lore 数值或前缀不一致。

## 33.6 构建顺序与验证边界

`add_divine_covenants.py` 必须晚于 `add_kabbalah_covenant.py`、`add_player_panel.py` 与
`polish_recent_ui.py`：它需要接管生命之树完成奖励、修改最终面板，并写入上位契约
消息槽。随后才运行 `make_boxes.py` 与全局姓名抛光，保证领取箱收录的是最终物品。

`check_kabbalah_covenant.py` 验证一份血契、十枚源质、唯一完成入口和每玩家输入冷却；
`check_divine_covenants.py` 验证旧约一次性奖励、Daath 消耗与汇聚、新约双式、赦免
入口、冷却刻数及权柄 HUD。动态探针另从真实 1.21.11 服务端确认签约、伤害、解约、
树体回收和多人归属。文本校验必须以生成器为源，不能只检查当前 `rpg/` 输出。

网页图鉴的玩家向数值集中在 `_divine.json`。`extract_items.py` 从最终 `give/extra` 抽取
血契、十源质、旧约、新约与真·十字架；`build_guide.py` 将这 14 件从普通消耗品中摘出、
...
`_tools/check_debug_commands.py` 会在图鉴构建时检查重复项、函数是否存在、公开罪仆与
第一章调试入口是否漏收、内部 `*_worker` 是否误公开、罪仆中文名／职责是否与函数头
...
　　　　　→ `add_pact.py` + `add_squad.py` + `add_holy_items.py`
　　　　　→ `retype_longinus.py` + `modernize_legacy_weapons.py`
　　　　　→ `modernize_legacy_advanced.py` + `add_book.py` + `make_boxes.py`
　　　　　→ `opt_actionbar.py` + `fix_display.py` → `opt_mp.py`
　　　　　→ `opt_index.py` + `opt_holy.py` + `opt_type.py` + `opt_guard.py`
　　　　　→ `opt_hoist.py` + `opt_invert.py`
　　　　　　→ `opt_runtime_hotpaths.py`
...
　　　　　→ `rp_validate.py`
随后再跑数据包流程。部分共用内容生成器在材质构建时也会写数据侧入口；让
`build.sh` 最后运行，才能保证旧武器现代化、actionbar 收编和多人修正不会被覆盖。
