...
只加装饰，不动结构，也不碰锁定的深色调。词汇取自铜版画册页：

* **希腊回纹饰带** —— 每个章节顶部一条。上下两道细线，中间的齿一上一下
  交错半个周期，纯 CSS 渐变实现
* **浮雕徽章** —— 罗马数字放进斑岩底（`#43181C`，古典斑岩的暗酒红）的金环里，
  外面再套一圈镶口，像一枚嵌在册页上的宝石浮雕
* **月桂分隔** —— 内联 SVG 的对生桂枝，两侧引出渐隐细线；
  用在报头之下与世界观卷首之后
* **廊柱凹槽** —— 导航栏右缘加竖向凹槽
* **四角挂线** —— 每个章节四角的直角细线，像装裱好的版画

刻意**没有给卡片加装饰** —— 页面上有一百多张密集卡片，
再往上加纹样就成了噪音。装饰只落在版面家具上。

字体没有新增：Cinzel 本来就是图拉真柱式的罗马大写，正是新古典的那张脸，
只是把字距推到 .3em 让它更像碑铭。中文正文的节奏一点没动。

用浏览器量过：装饰层**没有引入任何横向溢出**
（开与关两次测量完全相同），四角挂线的内缩也改成 0 以杜绝这种可能。
