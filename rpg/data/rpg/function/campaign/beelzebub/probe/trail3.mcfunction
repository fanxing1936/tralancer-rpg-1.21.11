...
execute if score @s rpg_ch1_seen matches 40.. as @a[tag=rpg.ch1.current,distance=..2.8,sort=nearest,limit=1] if score @s rpg_ch1_id = @e[type=minecraft:marker,tag=rpg.ch1.point.active,limit=1] rpg_ch1_id run return run function rpg:campaign/beelzebub/point/trail3
