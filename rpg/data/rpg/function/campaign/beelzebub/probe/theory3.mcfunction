...
execute if score @s rpg_ch1_seen matches 15 run playsound minecraft:block.amethyst_block.chime player @a[tag=rpg.ch1.current,distance=..8] ~ ~ ~ 0.35 1.25
execute if score @s rpg_ch1_seen matches 30.. as @a[tag=rpg.ch1.current,distance=..2.8,sort=nearest,limit=1] if score @s rpg_ch1_id = @e[type=minecraft:marker,tag=rpg.ch1.point.active,limit=1] rpg_ch1_id run return run function rpg:campaign/beelzebub/choice/theory3
