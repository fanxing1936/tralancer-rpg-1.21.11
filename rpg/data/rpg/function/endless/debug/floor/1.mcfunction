...
scoreboard players set @e[type=minecraft:marker,tag=rpg.end.controller.current,limit=1] rpg_end_floor 1
...
tellraw @s ["",{"text":"[调试跳层] ","color":"#C28BE0","bold":true,"italic":false},{"text":"下一刻进入第 1 层；不会发放被跳过层数的奖励。","color":"#AAB4C3","bold":false,"italic":false}]
