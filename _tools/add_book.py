...
def build_pages():
    ...
    p.append(page(
        *vol("七", "第一章·空缺者"), rule(),
        t("玩家面板里打开"), t("[第一章·空缺者]", "#B5D957", True),
        t("，接受维斯珀城调查令。楔子从第十三声钟与互相矛盾的粮册、墓地记录开始。完整流程约 30–60 分钟；队伍共享场景与 Boss 进度，调查、首通和职业档案跟随个人。"),
        gap(),
        aside("前线吃人，后方负责忘记那些被吃掉的人。")))

    p.append(page(
        *vol("七", "会回家的死者"), rule(),
        t("先观察餐桌记录、死亡登记与钟楼检材，不要立刻把答案写成疫病或亡灵。名册若写出你的名字，就已经不是替陌生人冒险。"),
        t("五名罪仆之后，让口粮、尸体、灭口三条路线彼此指认。每次案情复盘都记录已知、矛盾与下一步；重开章节档案可随时查看当前梗概。"),
        gap(),
        aside("空缺者记得自己是谁，却不知道“自己”是什么意思。")))

    p.append(page(
        *vol("七", "别西卜的真名"), rule(),
        t("粮仓的余烬、蝇茧和腐败祭品只会建立真名假说。进入 Boss 第一阶段后，携带圣器亲历五招中的任意三种不同见证，才会正式确认"),
        t("别西卜", "#5A6B1E", True), t("。"),
        gap(),
        aside("弱点媒介：毒马铃薯·腐宴残食。")))

    p.append(page(
        *vol("七", "万蝇腐宴"), rule(),
        t("一、镇压到 420 / 700 生命。\n"),
        t("二、点燃图腾、宣读真名。\n"),
        t("三、用媒介和器具把稳定度推到 100。\n"),
        t("四、选择消灭、放逐、封印或契约。"),
        gap(),
        aside("稳定度归零只会重立仪式，不会错误掉出完整罪器。")))

    p.append(page(
        *vol("七", "缺失的见证"), rule(),
        t("教廷故意撕掉了判词中的"), t("见证人印", "#D4AF37", True),
        t("。四种裁决都有效，却只能击中卡西安留下的空壳；别西卜会逃走，你得到的是所选路线的裁决残响，不是完整核心或真实契约。"),
        gap(),
        aside("裁决的是名字，不是饥饿。")))

    p.append(page(
        *vol("七", "成为边缘者"), rule(),
        t("救下还能说出自己姓名的米拉，你会第一次释放未经许可的魔力。教廷随后给出两个选择：被当作污染源处决，或签下终身征召的边缘者入院令。"),
        t("结案后在审判、守护、秘仪中完成第一次强化，并解锁路西法的王冠失窃案。"),
        gap(),
        aside("你没有被神选中。你只是为了活下去，成为了武器。")))

    p.append(page(
        *vol("七", "门从何处来"), rule(),
        t("驱净空缺者，有时会留下一份教区名册。一个空壳散去，会牵出更长的线索；带到远离聚落的旷野再打开。"),
        gap(),
        t("证得任一柱真名后，会有人来找你，并留下七柱回廊的信物。"),
        gap(),
        aside("线索认得见过它的人。")))
