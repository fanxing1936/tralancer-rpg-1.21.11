...
def build_pages():
    ...
    p.append(page(
        *vol("一", "圣器护着你"), rule(),
        t("身上带着圣器时，恶魔"), t("蒙不住你的眼", None, True),
        t("：黑暗与失明落不下来，其余 debuff（中毒、凋零、饥饿、缓慢…）时间减半。"),
        gap(),
    ...
    p.append(page(
        *vol("二", "它出来了"), rule(),
        t("九十秒走完，有东西从你身上挣出去。你看不见它 —— "
          "只能看见它周身那团黑烟。它待三十秒，而且"),
        t("每位三招", None, True),
        t("，出手时掷一招 —— 同一位打两次不会长得一样。"),
        gap(),
        aside("它死的时候会炸；自己散掉的那一只不会。")))
