...
particle dust_color_transition{{from_color:{EMBER},to_color:{DEEPRED},scale:2}} ~ ~1 ~ 0.6 0.5 0.6 0.06 22
particle lava ~ ~0.8 ~ 0.5 0.4 0.5 0 6
playsound minecraft:item.axe.scrape player @a[distance=..18] ~ ~ ~ 1 1.6

# type=!evoker_fangs 不能少：不排除的话，这一轮会在**上一轮召出的每一只
# 尖牙**旁边再召一只 —— 第 n 代生第 n+1 代，六轮下来是指数爆炸。
execute as @e[distance=0.1..3.5,type=!player,type=!minecraft:item,type=!minecraft:experience_orb,type=!minecraft:evoker_fangs] at @s run summon minecraft:evoker_fangs ~ ~ ~ {{Warmup:0,Tags:["rpg.saw.fang"],attributes:[{{id:"scale",base:2.2f}}]}}
...
execute as @e[distance=0.1..3.5,type=!player,type=!minecraft:item,type=!minecraft:experience_orb,type=!minecraft:evoker_fangs] at @s run damage @s 5 minecraft:player_attack by @p[tag=rpg.h.dawn_tag1]
# 熔岩锯齿咬过之后伤口一直在烧 —— 六轮之间的空档由灼烧填满，
# 目标因此是持续掉血，而不是每 10 刻才动一次。
execute as @e[distance=0.1..3.5,type=!player,type=!minecraft:item,type=!minecraft:experience_orb,type=!minecraft:evoker_fangs] at @s run data merge entity @s {{Fire:120s}}
execute as @e[distance=0.1..3.5,type=!player,type=!minecraft:item,type=!minecraft:experience_orb,type=!minecraft:evoker_fangs] at @s run effect give @s minecraft:glowing 3 0 true
execute as @e[distance=0.1..3.5,type=!player,type=!minecraft:item,type=!minecraft:experience_orb,type=!minecraft:evoker_fangs] at @s run particle trial_spawner_detection ~ ~1 ~ 0.4 0.5 0.4 0.1 10
execute as @e[distance=0.1..3.5,type=!player,type=!minecraft:item,type=!minecraft:experience_orb,type=!minecraft:evoker_fangs] at @s run playsound minecraft:entity.blaze.burn hostile @a[distance=..18] ~ ~ ~ 1 0.8
"""
